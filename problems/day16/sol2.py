...
for i in range(len(M)):
    ans = max(ans, solve_problem(i, 0, 0))
    ans = max(ans, solve_problem(i, len(M[0]) - 1, 2))

for i in range(len(M[0])):
    ans = max(ans, solve_problem(0, i, 1))
    ans = max(ans, solve_problem(len(M) - 1, i, 3))
